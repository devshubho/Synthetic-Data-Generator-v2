...
class CustomGenerator:
    ...
    def generate(self, n: int, preserve_correlations: bool = True) -> pd.DataFrame:
        ...
        for col, model in self.models.items():
            if model['type'] == 'numeric':
                ...
                data[col] = np.clip(samples, model['min'], model['max'])
            
            elif model['type'] == 'datetime':
                min_ts = model['min'].timestamp()
                max_ts = model['max'].timestamp()
                timestamps = np.random.uniform(min_ts, max_ts, n)
                data[col] = pd.to_datetime(timestamps, unit='s')
            
            else:
                data[col] = np.random.choice(model['values'], n, p=model['probs'])
    ...
    def _apply_correlations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_cols = self.sample.select_dtypes(include=[np.number]).columns
                ...
                for i, col1 in enumerate(numeric_cols):
                    for j, col2 in enumerate(numeric_cols):
                        if i < j:
                            ...
                            if abs(corr) > 0.3:
                                df[col2] = df[col2] + corr * (df[col1] - df[col1].mean())
            except Exception:
                pass
